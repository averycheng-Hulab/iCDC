## Fig 4g-h | B cell UMAP + B composition
...
infile <- file.path("Figs", "data", "B_plot_input.csv")
stopifnot(file.exists(infile))

outdir <- file.path("Figs", "outputs")
dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

outfile_umap <- file.path(outdir, "Fig4g_B_umap_by_Subtype.pdf")
outfile_comp <- file.path(outdir, "Fig4h_B_cellstat.pdf")
...
clusters <- c("B-Naive","B-Act1","B-Act2","B-ActMem","B-Plasma","B-Mye")
cols <- c("#a9d6e8","#2799c8","#82cf6c","#41af2e","#f6c06e","#ff9400")
pal_b <- setNames(cols, clusters)

## ---------------------------
## Fig 4g | UMAP colored by subtype
## ---------------------------
df_umap <- df %>%
...
p_comp <- ggplot(plot_df, aes(x = Treatment, y = percent, fill = cluster_label)) +
  ...
  scale_fill_manual(values = pal_b, breaks = clusters, drop = FALSE) +
  theme_bw() +
  labs(x = NULL, y = "Percentage (%)", fill = NULL, title = "B composition") +
